...
int encontrarSubconjunto(Subconjunto subconjunto[], int i) {
  if (subconjunto[i].pai != i) // Encontra a raiz e a torna pai de i.
    subconjunto[i].pai = encontrarSubconjunto(subconjunto, subconjunto[i].pai);
...
void unirSubconjunto(Subconjunto subconjunto[], int x, int y) {
  int raizx = encontrarSubconjunto(subconjunto, x); // Encontra os subconjunto de x.
  int raizy = encontrarSubconjunto(subconjunto, y); // Encontra os subconjunto de y.
...
int comparaDecre(const void *x, const void *y) {
  Aresta *ArrestaX = (Aresta *)x;
  Aresta *ArrestaY = (Aresta *)y;
  return ArrestaX -> custo < ArrestaY -> custo;
}
...
int Kruskal(Grafo *grafo) {
  ...
  while (e < V - 1 && i < grafo->E) {
    ...
    int u = encontrarSubconjunto(subconjunto, proximo_aresta.origem); // Determina x como o Subconjunto de origem.
    int v = encontrarSubconjunto(subconjunto, proximo_aresta.destino); // Determina y como o Subconjunto de destino.
...
int main() {
  ...
  qsort(grafo->aresta, grafo->E, sizeof(grafo->aresta[0]), comparaDecre);
